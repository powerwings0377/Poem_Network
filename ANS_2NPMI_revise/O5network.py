...
def analyze_correlation_metrics(df):
    ...
    # 计算各指标的相关性
    correlation_matrix = df[['NPMI值', 'PMI值', '联合概率', '共现文档数']].corr()
    print("各指标相关系数矩阵:")
    ...
    # NPMI vs 共现文档数散点图
    scatter = axes[1, 1].scatter(df['共现文档数'], df['NPMI值'],
                                 c=df['联合概率'], cmap='viridis',
                                 alpha=0.6, s=30)
    axes[1, 1].set_xlabel('共现文档数')
    axes[1, 1].set_ylabel('NPMI值')
    axes[1, 1].set_title('NPMI vs 共现文档数')
...
if __name__ == "__main__":
    # 替换为你的文件路径
    file_path = "output_npmi_盛唐_20260304_152936_npmi.csv"
    ...
    try:
        ...
        print("词1,词2,NPMI值,PMI值,共现文档数,词1文档频次,词2文档频次,词1概率,词2概率,联合概率")
